from legacy.services.tool_registry import tool_registry
...
from domain.models.game_result import GameResult, QuickReplyAction

async def handle_version(session, user_id: str, text: str):
    return GameResult(
        text="Cyborg Architecture v0.3 (Hybrid Mode)",
        quick_replies=[
            QuickReplyAction(label="Check Status", action_data="status")
        ],
...
async def ai_fallback(session, user_id: str, text: str):
    ...
    from legacy.services.ai_service import ai_router
...
def setup_dispatcher():
    ...
    # Version (Hybrid Test)
    dispatcher.register(is_exact("version"), handle_version)
