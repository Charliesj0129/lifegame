...
class QuestService:
    ...
    async def create_new_goal(self, session: AsyncSession, user_id: str, goal_text: str):
        ...
        try:
            ...
            fallback_quests = [
                {"title": "建立今天的學習清單", "desc": "列出 3 個今日要完成的小步驟（10 分鐘）", "difficulty": "E"},
                {"title": "完成一個最小練習題", "desc": "選 1 題簡單題並完成（20 分鐘）", "difficulty": "D"},
                {"title": "寫下回顧與下一步", "desc": "記錄 3 行心得與明日計畫（10 分鐘）", "difficulty": "E"},
            ]
    ...
    async def _generate_daily_batch(self, session: AsyncSession, user_id: str, time_context: str = "Daily"):
        ...
        try:
            ...
            if isinstance(ai_data, dict) and not quest_list:
                 ...
                 if "title" in ai_data: quest_list = [ai_data]
            ...
            fallback_templates = [
                {"title": "系統重啟", "desc": "5 分鐘深呼吸或走動（備援）", "diff": "E", "xp": 10},
                {"title": "資料同步", "desc": "記錄 3 句今日進度（備援）", "diff": "D", "xp": 20},
                {"title": "硬體維護", "desc": "整理桌面 10 分鐘（備援）", "diff": "D", "xp": 20}
            ]
