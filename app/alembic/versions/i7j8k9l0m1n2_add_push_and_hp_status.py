...
def upgrade() -> None:
    ...
    op.add_column('users', sa.Column('push_enabled', sa.Boolean(), nullable=True, server_default='true'))
    op.add_column('users', sa.Column('push_timezone', sa.String(), nullable=True, server_default='Asia/Taipei'))
    
    # Add HP status fields (hp and max_hp already exist)
    op.add_column('users', sa.Column('hp_status', sa.String(), nullable=True, server_default='HEALTHY'))
