...
async def get_dashboard(request: Request, user_id: str, db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        if not user:
            ...
            return templates.TemplateResponse("error.html", {"request": request, "error": "User Not Found"}, status_code=404)
        ...
        # 4. Inventory
        from app.models.gamification import UserItem, Item
        ...
        return templates.TemplateResponse("error.html", {"request": request, "error": f"Internal Error: {e}"}, status_code=500)
