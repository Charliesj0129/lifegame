...
async def test_loot_drop_end_to_end(db_session):
    ...
    with (
        patch.object(loot_service, "_roll_for_drop", return_value=True),
        ...
        res = await db_session.execute(
            select(UserItem).where(UserItem.user_id == "u_loot_test")
        )
...
async def test_status_card_rendering():
    ...
    user = User(
        id="u_stat", name="Hero", level=5, str=10, int=5, vit=5, wis=5, cha=5, gold=100
    )

    msg = flex_renderer.render_status(user)
    assert msg.alt_text.startswith("戰術系統") or msg.alt_text.startswith(
        "Tactical OS"
    )  # Support Chinese
