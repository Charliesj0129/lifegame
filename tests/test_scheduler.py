...
async def test_morning_push_calls_quest_service():
    ...
    with patch("app.services.scheduler.quest_service") as mock_quest_service, \
         patch("app.services.scheduler.get_messaging_api") as mock_api, \
         patch.object(DDAScheduler, "_get_or_create_profile", new_callable=AsyncMock) as mock_profile, \
...
async def test_push_respects_user_preference():
    ...
    with patch("app.services.scheduler.AsyncSessionLocal") as mock_session_local, \
         patch.object(DDAScheduler, "_process_user", new_callable=AsyncMock) as mock_process, \
