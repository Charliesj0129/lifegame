...
async def test_quest_ui_render():
    print("\n--- Testing Quest UI ---")
    quests = [
        Quest(id="q1", title="Test Q1", description="D", difficulty_tier="E", xp_reward=20, status="ACTIVE"),
        Quest(id="q2", title="Test Q2", description="D", difficulty_tier="C", xp_reward=50, status="DONE")
    ]
...
async def slow_ai_response(*args, **kwargs):
    ...
    return {"img_url": "http://fake", "text": "Fail", "quests": [{"title": "System Reboot", "desc": "Fallback", "diff": "A", "xp": 100}]}
...
async def test_ai_timeout_fallback(db_session):
    ...
    with patch.object(ai_engine, 'generate_json', side_effect=slow_ai_response):
        ...
        # Mock dependencies
        with patch("app.services.rival_service.RivalService.get_rival", new_callable=AsyncMock) as m1, \
             patch("app.services.user_service.UserService.get_user", new_callable=AsyncMock) as m2:
