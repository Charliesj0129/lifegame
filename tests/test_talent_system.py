...
async def test_learn_talent_success(db_session):
    ...
    success, msg = await talent_service.learn_talent(
        db_session, "test_user", "STR_01_BLOODLUST"
    )
    ...
    user = (
        await db_session.execute(select(User).where(User.id == "test_user"))
    ).scalar_one()
...
async def test_learn_talent_requires_parent(db_session):
    ...
    success, msg = await talent_service.learn_talent(
        db_session, "test_user", "STR_02_IRON_WILL"
    )
...
async def test_get_player_class_empty(db_session):
    ...
    class_type, class_name, emoji = await talent_service.get_player_class(
...
async def test_check_penalty_evasion_with_talent(db_session):
    ...
    evade_count = 0
    for _ in range(100):
        evaded, msg = await talent_service.check_penalty_evasion(
            db_session, "test_user"
        )
        if evaded:
            evade_count += 1
