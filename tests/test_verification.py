...
async def test_verify_text_report():
    with patch(
        ...
        result = await verification_service.verify_text_report(
            "I ran 5km today.", "Run 5km"
        )
...
async def test_verify_image_report():
    with patch(
        "app.services.ai_engine.ai_engine.analyze_image", new_callable=AsyncMock
    ) as mock_vision:
        ...
        result = await verification_service.verify_image_report(
            fake_image, "image/jpeg", "Eat a Salad"
        )
