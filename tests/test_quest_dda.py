...
async def test_dda_trigger():
    ...
    mock_result.scalars.return_value.first.side_effect = [
        ...
        None, # Goal
        DailyOutcome(user_id="u1", done=False, date=datetime.date.today() - datetime.timedelta(days=1)) # Failed
    ]
    ...
    with patch("app.services.ai_engine.ai_engine.generate_json", new_callable=AsyncMock) as mock_ai:
        mock_ai.return_value = [
            {"title": "Easy Recovery", "diff": "E", "xp": 10}
        ]
...
async def test_serendipity_trigger():
    ...
    with patch("random.random", return_value=0.1), \
        ...
        mock_ai.return_value = [{"title": "Normal Task", "desc": "Desc", "diff": "D", "xp": 20}] 
