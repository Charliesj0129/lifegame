...
async def test_hp_capped_at_max(db_session):
    ...
    user = User(id="test_cap", name="Capped User", hp=95, max_hp=100, hp_status="HEALTHY")
    db_session.add(user)
    await db_session.commit()
...
async def test_get_hp_display(db_session):
    ...
    user = User(id="test_display", name="Display User", hp=75, max_hp=100, hp_status="HEALTHY")
    db_session.add(user)
    await db_session.commit()
