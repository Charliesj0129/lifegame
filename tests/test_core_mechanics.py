...
async def test_webhook_creates_user(db_session):
    ...
    with (
        ...
        with patch.object(
            webhook_handler.parser.signature_validator, "validate", return_value=True
        ):
            ...
            mock_event = MessageEvent(
                ...
                message=TextMessageContent(
                    id="mid", type="text", text="Gym 1 hour", quoteToken="q_token"
                ),
            )

            with patch.object(
                webhook_handler.parser, "parse", return_value=[mock_event]
            ):
