...
async def test_router_status_intent(db_session):
    ...
    with (
        ...
        patch(
            "app.services.rival_service.RivalService.get_or_create_rival",
            new_callable=AsyncMock,
        ) as mock_get_rival,
        patch(
        ...
        patch(
            "app.services.tool_registry.ToolRegistry.get_status", new_callable=AsyncMock
        ) as mock_tool_status,
    ):
...
async def test_router_inventory_fallback(db_session):
    ...
    with (
        ...
        patch(
            "app.services.tool_registry.ToolRegistry.log_action", new_callable=AsyncMock
        ) as mock_log_action,
    ):
...
async def test_router_multi_tool_calls(db_session):
    ...
    with (
        ...
        patch(
            "app.services.tool_registry.ToolRegistry.use_item", new_callable=AsyncMock
        ) as mock_use_item,
        patch(
            "app.services.tool_registry.ToolRegistry.log_action", new_callable=AsyncMock
        ) as mock_log_action,
        ...
        mock_gen_json.return_value = {
            ...
            "tool_calls": [
                ...
                {
                    "tool": "log_action",
                    "arguments": {"text": "I'm tired and going to sleep"},
                },
            ],
