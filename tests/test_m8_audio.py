...
class TestM8Audio(unittest.IsolatedAsyncioTestCase):
    async def test_webhook_levelup_audio(self):
        ...
        with (
            ...
            patch(
                "app.services.ai_service.ai_router.router", new_callable=AsyncMock
            ) as mock_router,
            ...
            patch(
                "app.api.webhook.user_service.get_or_create_user",
                new_callable=AsyncMock,
            ),
            patch(
                "app.services.rival_service.RivalService.process_encounter",
                new_callable=AsyncMock,
            ) as mock_rival_encounter,
        ):

            mock_rival_encounter.return_value = None
            mock_router.return_value = (
                TextMessage(text="Flex"),
                "log_action",
                {"leveled_up": True},
            )

            # Mock Audio Return
            mock_audio_msg = AudioMessage(
                original_content_url="https://audio", duration=1000
            )
            ...
            self.assertIsInstance(
                messages[1], AudioMessage, "Second message should be Audio"
            )
    ...
    async def test_briefing_audio(self):
        ...
        with (
            patch(
                "app.services.daily_briefing_service.get_messaging_api"
            ) as mock_get_api,
            patch("app.services.daily_briefing_service.AsyncSessionLocal") as mock_db,
            patch(
                "app.services.daily_briefing_service.rival_service.advance_daily_briefing",
                new_callable=AsyncMock,
            ) as mock_update_rival,
        ):
            ...
            self.assertIsInstance(
                messages[0], AudioMessage, "First message should be Audio"
            )
