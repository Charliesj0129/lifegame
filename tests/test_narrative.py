...
async def test_get_viper_comment():
    ...
    with (
        patch(
            "app.services.rival_service.rival_service.get_rival", new_callable=AsyncMock
        ) as mock_get_rival,
        patch(
            "app.services.ai_engine.ai_engine.generate_json", new_callable=AsyncMock
        ) as mock_ai,
        ...
        comment = await narrative_service.get_viper_comment(
            mock_session, "u1", "User failed test"
        )
