...
async def test_verify_location_approved(db_session):
    ...
    quest = (await db_session.execute(
        select(Quest).where(Quest.id == "QUEST_LOC_001")
    )).scalar_one()
...
async def test_verify_text_with_mock_ai(db_session):
    ...
    from sqlalchemy import select
    quest = (await db_session.execute(
        select(Quest).where(Quest.id == "QUEST_TEXT_001")
    )).scalar_one()
    ...
    with patch("app.services.ai_engine.ai_engine.verify_multimodal", new_callable=AsyncMock) as mock_verify:
...
async def test_verify_image_with_mock_ai(db_session):
    ...
    from sqlalchemy import select
    quest = (await db_session.execute(
        select(Quest).where(Quest.id == "QUEST_IMAGE_001")
    )).scalar_one()
