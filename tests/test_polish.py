...
async def test_verification_service_standardization():
    ...
    with patch("app.services.verification_service.ai_engine") as mock_ai:
        mock_ai.verify_multimodal = AsyncMock(return_value={
            "verdict": "APPROVED",
        ...
        # Test verify_text
        result = await verification_service.verify_text(mock_session, quest, "I ran 5k today")
        assert result["verdict"] == Verdict.APPROVED
        assert result["reason"] == "Clear evidence."
        
        # Test process_verification success flow
        with patch.object(verification_service, "auto_match_quest", return_value=quest), \
             patch.object(verification_service, "_complete_quest", return_value="Mission Complete"):
            
            q, v, msg = await verification_service.process_verification(mock_session, "u1", "I ran 5k", "TEXT")
...
async def test_user_service_habit_update():
    ...
    with patch("app.services.user_service.ai_engine") as mock_ai, \
        ...
        mock_ai.analyze_action = AsyncMock(return_value={
            "stat_type": "STR",
            "difficulty_tier": "C",
            "narrative": "Good job."
        })
        ...
        mock_session.execute.side_effect = [mock_result_user, mock_result_habits, MagicMock()]
        
        # Run process_action
        result = await user_service.process_action(mock_session, "u1", "I went to the gym")
...
async def test_quest_service_dda_push():
    ...
    with patch("app.services.quest_service.ai_engine") as mock_ai, \
