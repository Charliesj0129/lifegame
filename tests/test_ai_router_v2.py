...
async def test_router_chain_of_thought():
    ...
    with (
        ...
        patch(
            "app.services.rival_service.rival_service.get_or_create_rival",
            new_callable=AsyncMock,
        ) as mock_get_rival,
        patch(
            "app.services.ai_service.AIService._get_history", new_callable=AsyncMock
        ) as mock_hist,
        ...
        with (
            patch(
                "app.services.tool_registry.tool_registry.use_item",
                new_callable=AsyncMock,
            ) as mock_use,
            patch(
                "app.services.tool_registry.tool_registry.log_action",
                new_callable=AsyncMock,
            ) as mock_log,
        ):
            ...
            # Execute
            msg, tool, data = await AIService.router(
                mock_session, "u1", "Heal and sleep"
            )
