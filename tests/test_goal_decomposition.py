...
async def test_create_new_goal_decomposition():
    ...
    ai_response = {
        "milestones": [
            {"title": "閱讀第一章", "desc": "開始閱讀", "difficulty": "C"}
        ],
        "daily_habits": [
            {"title": "每天閱讀 10 分鐘", "desc": "每日固定時段"}
        ]
    }
    ...
    with patch("app.services.ai_engine.ai_engine.generate_json", new_callable=AsyncMock) as mock_ai:
        ...
        # Execute
        goal, plan = await quest_service.create_new_goal(mock_session, "u1", "Learn Python")
        ...
        has_quest = any(isinstance(obj, Quest) and obj.title == "閱讀第一章" for obj in added_objects)
        has_habit = any(isinstance(obj, HabitState) and obj.habit_name == "每天閱讀 10 分鐘" for obj in added_objects)
